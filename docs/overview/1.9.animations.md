...
LiveCharts smoothly animates any change automatically. We have dedicated a lot of hours building a small (and sufficient) framework 
...
{{~ if xaml ~}}
Xaml already provides an easy way to write the `TimeSpan` type directly in your view, using the following format `hh:mm:ss`

...
AvaloniaUI or WPF. LiveCharts defines its own class, which is optimized for the library's use case.
...
object called `Paint`. The LiveCharts `Paint` is a wrapper for the SkiaSharp `Paint`, but it adds some features such as animations and 
has an API that feels more like WPF. It should feel easier to manipulate a LiveCharts `Paint` from the perspective of a front-end developer.
...
to indicate how things will render in the user interface. Take a look at the following samples:

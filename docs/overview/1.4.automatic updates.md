...
LiveCharts is listening for any change in your data. The library is able to update automatically 
...
You can call the `CollectionDeepObserver.Dispose` method if you are facing memory leaks, but that is not the 
...
the user interface. However, there are special cases where it is up to the developer whether the chart will update or 
not, since the implementation of these interfaces depends on the data you provide to the library. This is 
especially important to understand for the `Chart.Series` and `Series.Values` properties.

## Enable automatic updates for the Series.Values and Chart.Series properties

Automatic updates for most cases should not impact performance significantly, so feel free to enable them. LiveCharts 
...
the changes of an object in the user interface. LiveCharts also uses this interface to detect when a property in your 
...
Finally, you can also plot an object defined in your application (see [Mappers](./1.5.mappers.md)), and the 
logic is the same. The library will be able to reflect the changes in the user interface just when your 
object implements `INotifyPropertyChanged`. Let's take the following `City` class as an example.
...
// Let's now change the City class definition to implement INotifyPropertyChanged
